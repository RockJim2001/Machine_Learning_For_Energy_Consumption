...
if __name__ == '__main__':
    ...
    """================================算法参数设置============================="""
    myAlgorithm = ea.moea_MOEAD_archive_templet(problem, population)  # 实例化一个算法模板对象
    ...
    dir_path = os.path.join(ROOT_PATH, 'result_change', parent_dir, DATASET_NAME.split('.')[0], 'MOEADDRA')
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    df1.to_csv(os.path.join(dir_path, f'Variable_MOEADDRA_{ML_method}.csv'), header=None, index=None)
    df2.to_csv(os.path.join(dir_path, f'Objective_MOEADDRA_{ML_method}.csv'), header=None, index=None)
