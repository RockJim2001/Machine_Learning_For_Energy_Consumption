...
# 这里选择需要执行的机器学习方法
ML_method = 'MultilayerPerceptronRegressor'
...
if __name__ == '__main__':
    ...
    df1.to_csv(os.path.join(ROOT_PATH, 'result', DATASET_NAME.split('.')[0], f'Variable_pps_MOEAD_DE_{ML_method}.csv'),
               header=None, index=None)
    df2.to_csv(os.path.join(ROOT_PATH, 'result', DATASET_NAME.split('.')[0], f'Objective_pps_MOEAD_DE_{ML_method}.csv'),
               header=None, index=None)
