...
# 读取训练好的机器学习模型
def load_AdaBoost_model(path, x):
    # 加载模型
...
class MyProblem(ea.Problem):  # 继承Problem父类
    ...
    def aimFunc(self, pop):  # 目标函数
        ...
        for i in range(popsize):
            ...
            # 计算目标函数值
            f1, f2, f3, f4 = load_AdaBoost_model(os.path.join(ROOT_PATH, 'result', DATASET_NAME.split('.')[0],
                                                              'AdaBoostRegressor/AdaBoostRegressor.pth'), X)
            F1[i, 0] = f1
            F2[i, 0] = f2
            F3[i, 0] = f3
            F4[i, 0] = f4
