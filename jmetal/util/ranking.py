...
class FastNonDominatedRanking(Ranking[List[S]]):
    ...
    def compute_ranking(self, solutions: List[S], k: int = None):
        ...
        :param k: Number of individuals(希望保留的个体数目).
        return 每层个体组成的列表 [[第1层个体],[第2层个体],[第3层个体],...,[临界层个体]]
        """
        ...
        # front[k] contains the list of solutions belonging to front k
        ...
        if k:
            ...
            for i, front in enumerate(self.ranked_sublists):
                ...
                if count >= k:
                    ...
                    break
        # 每层个体组成的列表 [[第1层个体],[第2层个体],[第3层个体],...,[临界层个体]]
...
class StrengthRanking(Ranking[List[S]]):
    ...
    def compute_ranking(self, solutions: List[S], k: int = None):
        ...
        # strength(k) = | {j | j < - SolutionSet and k dominate j} |
        for i in range(len(solutions)):
        ...
        # rawFitness(k) = |{sum strength(j) | j <- SolutionSet and j dominate k}|
        for i in range(len(solutions)):
            for j in range(len(solutions)):
