...
class NaryRandomSolutionSelection(Selection[List[S], S]):
    ...
    def execute(self, front: List[S]) -> S:
        if front is None:
            raise Exception('交叉excute传入为None')
        if len(front) == 0:
            raise Exception('交叉excute传入为空')
        if len(front) < self.number_of_solutions_to_be_returned:
            raise Exception('交叉excute传入的个体数目 < 需返回的数量')

        # random_search sampling without replacement
        # random.sample(list, k)返回一个长度为k新列表，新列表存放list所产生k个随机唯一的元素
