...
# MOEAD-M2M交叉
class MOEAD_M2M_Mutation(Mutation[FloatSolution]):

    def __init__(self, gen: int, MaxGen: int, probability: float):
    ...
    def execute(self, solution: FloatSolution) -> FloatSolution:
        ...
        for i in range(solution.number_of_variables):
            ...
            if rand <= self.probability:
                ...
                rm = 0.15*rand * (1 - random.random() ** (-(1 - self.gen / self.MaxGen) ** 0.7))
                y = solution.variables[i] + rm*(solution.upper_bound[i] - solution.lower_bound[i])
                if y < solution.lower_bound[i]:
                    y = solution.lower_bound[i] + 0.5 * random.random() * (
                                solution.variables[i] - solution.lower_bound[i])
                if y > solution.upper_bound[i]:
                    y = solution.upper_bound[i] - 0.5 * random.random() * (
...
# FJSP——OS
class SwappingMutation(Mutation[IntegerSolution]):
    '''对OS序列 进行操作：生成两点，互换两点基因'''
    def __init__(self, probability: float):
        super().__init__(probability=probability)
    ...
    def execute(self, solution: IntegerSolution) -> IntegerSolution:
        rand = random.random()
        if rand <= self.probability:
            length = len(solution.variables[0])
            pos1 = random.randint(0, length - 1)
            pos2 = random.randint(0, length - 1)
            if pos1 == pos2:
                return solution
            else:
                solution.variables[0][pos2], solution.variables[0][pos1] = solution.variables[0][pos1], solution.variables[0][pos2]
        return solution

    def get_name(self):
        return 'OS SwappingMutation'

# FJSP——OS
class NeighborhoodMutation(Mutation[IntegerSolution]):
    '''对OS序列 进行操作：生成三点，邻域全排列变异'''
    def __init__(self, probability: float):
        super().__init__(probability=probability)
    ...
    def execute(self, solution: IntegerSolution) -> IntegerSolution:
        ...
        if rand <= self.probability:
            ...
            pos1, pos2, pos3 = sorted(random.sample(range(length),3))

            e1 = solution.variables[0][pos1]
            e2 = solution.variables[0][pos2]
            e3 = solution.variables[0][pos3]

            # 生成3个基因的全排列，并随机选取一种
            permutations = [*itertools.permutations([e1, e2, e3])]
            permutation = random.choice(permutations)

            solution.variables[0][pos1] = permutation[0]
            solution.variables[0][pos2] = permutation[1]
            solution.variables[0][pos3] = permutation[2]

        return solution

    def get_name(self):
        return 'OS NeighborhoodMutation'

# FJSP——MS
class HalfMutation(Mutation[IntegerSolution]):
    '''对MS序列 进行操作：选择一般基因替换为其他机器'''
    ...
    def execute(self, solution: IntegerSolution) -> IntegerSolution:
        rand = random.random()
        if rand <= self.probability:
            length = len(solution.variables[1])
            ...
            for job in jobs:
                for op in job:
                    if i in positions:
                        solution.variables[1][i] = random.randint(0, len(op) - 1)
                    i += 1
        return solution

    def get_name(self):
        return 'MS HalfMutation'

# FJSP——WS
class WSMutation(Mutation[IntegerSolution]):
    '''对MS序列 进行操作：选择一般基因替换为其他机器'''
    def __init__(self, probability: float, parameters):
        super().__init__(probability=probability)
        self.parameters = parameters
    ...
    def execute(self, solution: IntegerSolution) -> IntegerSolution:
        rand = random.random()
        if rand <= self.probability:
            length = len(solution.variables[2])

            jobs = self.parameters['jobs']  # 获取工件数
            ...
            for job in jobs:
                for op in job:
                    if i in positions:
                        solution.variables[2][i] = random.randint(0, self.parameters['VmNum']-1)
                    i += 1
        return solution
...
class FJSPMutaiton(Mutation[IntegerSolution]):
    '''OS,MS变异，不涉及到WS变异'''
    def __init__(self, probability: float, parameters):
        super().__init__(probability=probability)
        self.parameters = parameters
    ...
    def execute(self, solution: IntegerSolution) -> IntegerSolution:
        ...
        if random.choice([True, False]):
            offspring = SwappingMutation(self.probability).execute(solution)
        else:
            offspring = NeighborhoodMutation(self.probability).execute(solution)
        offspring = HalfMutation(self.probability, self.parameters).execute(offspring)
        return offspring

    def get_name(self):
        return 'FJSPMutation'

class FJSPLFMutaiton(Mutation[IntegerSolution]):
    '''OS,MS, WS变异'''
    def __init__(self, probability: float, parameters):
        super().__init__(probability=probability)
        self.parameters = parameters
    ...
    def execute(self, solution: IntegerSolution) -> IntegerSolution:
        ...
        if random.choice([True, False]):
            ...
            offspring = NeighborhoodMutation(self.probability).execute(solution)
        offspring = HalfMutation(self.probability, self.parameters).execute(offspring)
        offspring = WSMutation(self.probability, self.parameters).execute(offspring)
        return offspring

    def get_name(self):
        return 'FJSPMutation'

class MA2018Mutaiton(Mutation[IntegerSolution]):
    '''OS,MS, WS变异'''
    def __init__(self, probability: float, parameters):
        super().__init__(probability=probability)
        self.parameters = parameters
    ...
    def get_name(self):
        return 'MA2018Mutation'

class FJSPLF_Compared_Mutation(Mutation[IntegerSolution]):
    '''OS,MS, WS变异'''
