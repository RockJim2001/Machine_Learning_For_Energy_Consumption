...
def ranks(data: np.array, descending=False):
    ...
    :return: ranks, where ranks[k][j] == rank of the k-th row w.r.t the j-th column.
    """
    s = 0 if (descending is False) else 1

    # Compute ranks. (ranks[k][j] == rank of the k-th treatment on the j-th sample.)
    if data.ndim == 2:
...
def friedman_test(data):
    ...
    ..note:: Null Hypothesis(原假设): In a set of k (>=2) treaments (or tested algorithms), all the treatments are equivalent, so their average ranks should be equal.

    :param data: An (n x 2) array or DataFrame contaning the results. In data, each column represents an algorithm and, and each row a problem.
    :return p_value: The associated p-value.
    :return friedman_stat: Friedman's chi-square. 卡方
    """
    ...
    # avranks
    print("friedman_test平均排名:", avranks.tolist())
