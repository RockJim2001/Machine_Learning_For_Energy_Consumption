...
def generate_summary_from_experiment(input_dir: str, quality_indicators: List[QualityIndicator],
    ...
    * <base_dir>
    ...
    :return: None.
    """
    path = r'QualityIndicatorSummary.csv'
        ...
        for filename in filenames:
            ...
            if 'FUN' in filename:
                ...
                for indicator in quality_indicators:
                    ...
                    reference_front_file = r"C:\Users\Hangyu\Desktop\JmetalTB\TB1\data\instance.PF"
                    ...
                    # 当前前沿标准化
                    F1_max, F1_min = max(algorithm_front[:, 0]), min(algorithm_front[:, 0])
                    F2_max, F2_min = max(algorithm_front[:, 1]), min(algorithm_front[:, 1])
                    F3_max, F3_min = max(algorithm_front[:, 2]), min(algorithm_front[:, 2])
                    # pareto前沿标准化
                    f1_max, f1_min = max(indicator.reference_front[:, 0]), min(indicator.reference_front[:, 0])
                    f2_max, f2_min = max(indicator.reference_front[:, 1]), min(indicator.reference_front[:, 1])
                    f3_max, f3_min = max(indicator.reference_front[:, 2]), min(indicator.reference_front[:, 2])

                    # 当前前沿标准化
                    algorithm_front[:, 0] = (algorithm_front[:, 0] - F1_min) / (F1_max - F1_min)
                    algorithm_front[:, 1] = (algorithm_front[:, 1] - F2_min) / (F2_max - F2_min)
                    algorithm_front[:, 2] = (algorithm_front[:, 2] - F3_min) / (F3_max - F3_min)
                    # Pareto前沿标准化
                    indicator.reference_front[:, 0] = (indicator.reference_front[:, 0] - F1_min) / (F1_max - F1_min)
                    indicator.reference_front[:, 1] = (indicator.reference_front[:, 1] - F2_min) / (F2_max - F2_min)
                    indicator.reference_front[:, 2] = (indicator.reference_front[:, 2] - F3_min) / (F3_max - F3_min)
                    result = indicator.compute(algorithm_front)
                    #result = indicator.compute([solutions[k].objectives for k in range(len(solutions))])
